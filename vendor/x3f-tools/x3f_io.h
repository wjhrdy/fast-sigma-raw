...
typedef struct x3f_s {
  ...
  /* Merrill's developed pipeline applies its denoised residual after the
     camera matrix. Keep that transient mono gain map attached to the X3F so
     convert_data() can preserve the native ordering without global state. */
  float *merrill_residual_gain;
  uint16_t *merrill_residual_origin;
  uint32_t merrill_residual_rows;
  uint32_t merrill_residual_columns;
  uint32_t merrill_residual_gain_channels;
  uint32_t merrill_residual_channels;
  uint32_t merrill_residual_row_stride;
  double merrill_intermediate_bias;
} x3f_t;
